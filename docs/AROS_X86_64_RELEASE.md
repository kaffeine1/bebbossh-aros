...
1. **Entropy review.** The `mincrt` `randfill()` fallback (`src/rand.c`) is a
   best-effort self-contained mixer. **Reviewed and fixed** (pending build +
   self-test confirmation): the previous x86_64/mincrt path had `aros_rdtsc()`
   stubbed to `0` and `DateStamp` gated out, so per-call entropy was only
   addresses + a counter — not cryptographically random. The current `src/rand.c`
   enables inline-asm `rdtsc` (safe under `-nostdlib`: pure CPU instruction) and
   routes `DateStamp` through the mincrt-safe `bebbossh_aros_datestamp` wrapper
   so each call mixes ns-class CPU time + 20 ms-class system tick. Residual risk:
   not a CSPRNG; replace with an AROS CSPRNG if one becomes available. Closure
   action: build the x86_64 toolset and run `make -f Makefile.aros-x86_64
   run-tests` (testEd25519, testChacha20 cover the cipher paths that consume
   `randfill`); confirm signatures/keys generated by `bebbosshkeygen` look
   random (e.g. distinct across runs).
2. **Zero-delay password-auth churn.** x86_64 had a known intermittent
   password-auth failure under zero-delay connection churn
   (`BEBBOSSH_AROS_STRESS_DELAY=0`). **Candidate fix applied, pending
   validation:** `SshSession::login` (`src/sshsession.cpp` ~1013) previously
   reset the password cache on every login under x86_64/mincrt, defeating the
   caching layer and forcing a per-auth `Open` of the passwd file. Under churn
   the DOS file handler was hammered and intermittently failed → auth fail.
   The reset has been removed so the cache loads once per daemon lifetime
   (same behavior as i386). Closure action: rebuild x86_64, run
   `BEBBOSSH_AROS_STRESS_DELAY=0 scripts/aros-transfer-stress-test.sh` against
   the QEMU x86_64 VM and confirm 0 password-auth failures across the full
   iteration count. Adding/removing a user now requires a daemon restart on
   both i386 and x86_64, as it already did on i386.
