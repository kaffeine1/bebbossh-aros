# ----------------------------------------------------------------------
# edmul16.asm - 68000 assembly routine for Edwards curve multiplication (16-bit limbs)
#
# License:
...
#
# Contents:
#   - _edmul: multiplication routine for Edwards curve field elements (16-bit words)
#
# Notes:
#   - Uses MUL32 macro to emulate 32×32->4 multiplication.
...
.globl _edmul
_edmul:
    movem.l d2-d7/a3-a6,-(sp)     | Save registers
...
.L1:
    ...
    add.l   a5,a5
    add.l   a5,a5
    lea     32(a2,a5),a5          | Second operand pointer
...
.L0:
    move.l  d7,a6                 | Save low result
    move.l  (a4)+,d4              | Load first operand word
    move.l  -(a5),d7              | Load second operand word
    MUL32   d4,d7,d3,d1,d0        | 64-bit product in d3:d1        
    
    move.l  a6,d7                 | Restore low result
    ...
    add.l   a5,a5
    add.l   a1,a5
    add.w   #32,a5                | Adjust first operand pointer
    neg.l   d4
    lea     32(a2),a4             | Restore second operand pointer
    move.l  d4,a3
    move.l  d4,d0
    bgt.s   .L2
    bra     .L4
...
.L3:
    move.l  d7,a6
    move.l  (a5)+,d4              | Load word
    move.l  -(a4),d7              | Load word
    MUL32   d4,d7,d2,d3,d1        | 64-bit product in d2:d3
...
.L5:
    movem.l (sp)+,d2-d7/a3-a6     | Restore registers
    rts                           | Return
